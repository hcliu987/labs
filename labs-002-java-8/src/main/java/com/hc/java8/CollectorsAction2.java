...
import static com.hc.java8.CollectorsAction.menu;
...
public class CollectorsAction2 {
  ...
  private static void testGroupingByConcurrentWithFunctionAndCollector() {
    ...
    ConcurrentMap<Dish.Type, Double> collect = menu.stream()
        .collect(Collectors
            .groupingByConcurrent(Dish::getType, Collectors.averagingInt(Dish::getCalories)));
    Optional.ofNullable(collect).ifPresent(System.out::println);
  }
  ...
  private static void testGroupingByConcurrentWithFunctionAndSupplierAndCollector() {
    ...
    ConcurrentMap<Dish.Type, Double> collect = menu.stream().collect(Collectors
        .groupingByConcurrent(Dish::getType, ConcurrentSkipListMap::new,
