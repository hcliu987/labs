...
public class RabbitConfig {
  ...
  public static class DirectExchangeConfiguration {
    ...
    public Binding demo05Bing() {
      return BindingBuilder.bind(demo05Queue()).to(demo05Exchange())
    ...
    public BatchingRabbitTemplate batchingRabbitTemplate(ConnectionFactory factory) {
      ...
      BatchingStrategy batchingStrategy = new SimpleBatchingStrategy(batchSize, bufferLimit,
          timeout);
      //创建taskscheduler 对象，用于实现超时发送的定时器
      TaskScheduler taskScheduler = new ConcurrentTaskScheduler();
      //创建batchingRabbitTemplate对象
      BatchingRabbitTemplate rabbitTemplate = new BatchingRabbitTemplate(batchingStrategy,
          taskScheduler);
